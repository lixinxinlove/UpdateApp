...
import com.lixinxin.updateapp.http.EventResponseEntity;
...
public class GsonUtils {
	...
	public static Boolean getValueByKeyAsBool(String json, String key) {
		JsonObject obj = new JsonParser().parse(json).getAsJsonObject();
		Boolean value = obj.get(key).getAsBoolean();
		return value;
	}
	...
	public static EventResponseEntity asJSONToResponseEntity2(String json) {
		JsonObject obj = new JsonParser().parse(json).getAsJsonObject();
		String code = obj.get("code").getAsString();
		String message = obj.get("message").getAsString();
		JsonElement dataobj = obj.get("data");
